$(function () {
    ...
    function goTo(n, isAuto) {
        ...
        function finish() {
            ...
            $ul.off("transitionend webkitTransitionEnd", finish);
            running = false;
            if (!isAuto) startAuto();
        }
        $ul.off("transitionend webkitTransitionEnd").on("transitionend webkitTransitionEnd", finish);
        setTimeout(finish, 900); /* 안전망: transition 이벤트 미발생 시 강제 해제 */
